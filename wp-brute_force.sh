...
printf "${Cyan} [!] Started at: $(date | cut -d " " -f5)" && printf "\n"

for i in {1000..50000..1000}; do
	printf "${Yellow} [*] Starting $i iteration\n" && for y in $(cat /usr/share/wordlists/rockyou.txt | head -n$i | tail -1000); do 
		(
	...
	if [[ -e $stop_file ]]; then
		rm -f "$stop_file"
		printf "${Cyan} [X] Stopped at: $(date | cut -d " " -f5)"
		exit
	fi
